...
export function topClientesPorCategoria(
  ...
  // TODO(estudiante): esta consulta está incompleta a propósito.
  //
  // Objetivo: dentro de CADA categoría, numerar a los clientes del 1
  // en adelante según su total gastado en ESA categoría (el que más
  // gastó, puesto 1) — no un ranking global entre todas las categorías.
  //
  // Pistas:
  //   - ROW_NUMBER() OVER (PARTITION BY categoria ORDER BY total DESC, cliente_id ASC)
  //     reinicia la numeración en cada categoría (PARTITION BY) y ordena
  //     por total descendente (cliente_id ASC solo para desempatar de
  //     forma determinista).
  //   - SQLite (como Postgres) NO permite filtrar por una window
  //     function directamente en el WHERE del mismo SELECT — por eso
  //     "puesto" se calcula en la CTE clasificacion y se filtra
  //     DESPUÉS, en la consulta final (WHERE puesto <= ?).
  //
  // npm test debe fallar tal cual está esto (todos los clientes quedan
  // en puesto 1, así que WHERE puesto <= n no filtra nada de verdad) y
  // pasar en cuanto la numeración use ROW_NUMBER() real.
