...
export function descendientesDeCategoria(
  ...
  // TODO(estudiante): esta CTE está incompleta a propósito — le falta
  // el término RECURSIVO, así que solo devuelve la categoría de partida,
  // nunca sus descendientes.
  //
  // Objetivo: devolver TODAS las categorías del subárbol que empieza en
  // categoriaId (incluida ella misma), a CUALQUIER profundidad — no
  // solo sus hijos directos.
  //
  // Pista: falta añadir, dentro del paréntesis de "arbol", después del
  // SELECT base:
  //   UNION ALL
  //   SELECT c.id, c.nombre, c.categoria_padre_id
  //   FROM categorias AS c
  //   JOIN arbol AS a ON c.categoria_padre_id = a.id
  // Esa segunda parte se une A SÍ MISMA (a la propia CTE "arbol") una y
  // otra vez, añadiendo un nivel más del árbol en cada vuelta, hasta que
  // no encuentra más hijos.
  //
  // npm test debe fallar tal cual está esto (no llega a los nietos) y
  // pasar en cuanto se añada el término recursivo.
  const filas = db
    .prepare(
